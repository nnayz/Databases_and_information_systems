...
def ss2pl_schedule_s3():
    ...
    try:
        ...
        print("T2: Committing transaction")
        conn2.commit()

        # Now T1 can acquire locks and proceed with both updates
        print("T2: Committing and releasing locks")
        conn2.commit()

        # Now T1 can acuire locks and proceed with both updates
        print("T1: Acquiring write lock on x")
        row = acquire_lock(cursor1, "x", WRITE_LOCK)
        if row:
            print(f"T1: Acquired write lock on x")
            cursor1.execute("UPDATE items SET value = 100 WHERE name = 'x'")
        ...
        print("T1: Acquiring write lock on y")
        row = acquire_lock(cursor1, "y", WRITE_LOCK)
        if row:
            print(f"T1: Acquired write lock on y")
        ...
        # T2 acquires new locks for operations
        conn2 = get_connection(isolation_level="READ COMMITTED")
        cursor2 = get_cursor(conn2)
