...
public class MovieService extends MovieServiceBase {
	...
	public FindIterable<Document> getBestMovies(int minVotes, double minRating, int limit) {
		...
		FindIterable<Document>  result = movies.find(and(gt("votes", minVotes), gt("rating", minRating)))
                     .sort(Sorts.orderBy(Sorts.descending("votes"), Sorts.descending("rating")))
                     .limit(limit);
		return result;
	}
	...
	public FindIterable<Document> getByGenre(String genreList, int limit) {
		...
		//TODO : implement
		FindIterable<Document>  result = movies.find(new Document("genre", new Document("$all", genres))).limit(limit);
		return result;
	}
	...
	public FindIterable<Document> searchByPrefix(String titlePrefix, int limit) {
		//TODO : implement
		Pattern pattern = Pattern.compile("^" + Pattern.quote(titlePrefix) + ".*", Pattern.CASE_INSENSITIVE);
		Document prefixQuery = new Document("title", pattern);;
	...
	public void saveMovieComment(String id, String comment) {
		// TODO implement
		Object key = org.bson.types.ObjectId.isValid(id) ? new ObjectId(id) : id;
	...
	public FindIterable getByTweetsKeywordRegex(String keyword, int limit) {
		...
		Document query = new Document("tweets.text", pattern);
	...
	public FindIterable<Document>  getGeotaggedTweets(int limit) {
		//TODO : implement
		FindIterable<Document>  result = tweets.find(exists("coordinates", true)).limit(limit);
;
		return result;
	}
	...
	public void saveFile(String name, InputStream inputStream, String contentType) {
		...
		System.out.println("Stored in GridFS under _id = " + fileId);
